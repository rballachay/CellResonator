...
# -*- coding: utf-8 -*-
"""
Created on Sat Jul 10 14:08:31 2021
...
path = '/Users/RileyBallachay/Documents/Fifth Year/Work for Dr.Piret/Histogram_Slices/sliced_may10.csv'
...
txt = '/Users/RileyBallachay/Documents/Fifth Year/Work for Dr.Piret/Resonator Data/May10_2021.csv'
...
x_data = time[425:1150]/60

ax.plot(x_data,scaler.fit_transform(scipy.ndimage.gaussian_filter1d(means[425:1150],sigma=.01).reshape(-1,1)),'maroon')
...
gauss_kernels = np.arange(10,100,90)
delays = np.arange(10,100,90)
#gauss_kernels = [57]
#delays = [24]
...
for (i,delay) in enumerate(delays):
    for (j,gauss) in enumerate(gauss_kernels):
        ...
        a, b = 10,2
        mean, var, skew, kurt = gamma.stats(a, b, moments='mvsk')
        x = np.linspace(gamma.ppf(0.01, a, b),gamma.ppf(0.99, a, b), 200)
        betadist = gamma.pdf(x, a, b)
        plt.figure()
        plt.plot(x,betadist,'k')
        plot_data = scaler.fit_transform(scipy.ndimage.convolve1d(image_data[:,1],betadist)[start:stop].reshape(-1,1))
