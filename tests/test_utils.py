    ...
    for key_1 in  data_items:
        for key_2 in data_items[key_1]['data']:
            for key_3 in data_items[key_1]['data'][key_2]:
                data_items[key_1]['data'][key_2][key_3] = data_items[key_1]['data'][key_2][key_3].to_dict()
...
def test_process_config_2files(file=TEST_2FILES):
    ...
    for key_1 in  data_items_basis:
        for key_2 in data_items[key_1]['data']:
            for key_3 in data_items[key_1]['data'][key_2]:
                df_basis = pd.DataFrame.from_dict(data_items_basis[key_1]['data'][key_2][key_3])
...
def test_make_process_config_1file(file=TEST_1FILE):
...
def test_make_process_config_conc(file=TEST_CONC):
    data_items = process_config(file,1)
...
def test_process_config_conc(file=TEST_CONC):
    ...
    for key_1 in  data_items['concentration']['data']:
        for key_2 in data_items['concentration']['data'][key_1]:
            df_basis = pd.DataFrame.from_dict(data_items_basis['concentration']['data'][key_1][key_2])
            df_basis.index=df_basis.index.astype('int64')
